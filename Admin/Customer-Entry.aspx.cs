...
public partial class Admin_Customer_Entry : System.Web.UI.Page
{
    ...
    protected void DropDownList2_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
             SqlConnection con1 = new SqlConnection(ConfigurationManager.AppSettings["connection"]);
            SqlCommand CMD = new SqlCommand("select * from Customer_Entry where Com_Id='" + company_id + "' ORDER BY Custom_Code asc", con1);
            DataTable dt1 = new DataTable();
            con1.Open();
            SqlDataAdapter da1 = new SqlDataAdapter(CMD);
            da1.Fill(dt1);
    ...
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["connection"]);
        SqlCommand CMD = new SqlCommand("select * from Customer_Entry where Custom_Name='"+TextBox1.Text+"' and Com_Id='" + company_id + "' ORDER BY Custom_Code asc", con);
        DataTable dt1 = new DataTable();
        SqlDataAdapter da1 = new SqlDataAdapter(CMD);
        da1.Fill(dt1);
        GridView1.DataSource = dt1;
    ...
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["connection"]);
        SqlCommand CMD = new SqlCommand("select * from Customer_Entry where Mobile_no='" + TextBox14.Text + "' and Com_Id='" + company_id + "' ORDER BY Custom_Code asc", con);
        DataTable dt1 = new DataTable();
        SqlDataAdapter da1 = new SqlDataAdapter(CMD);
        da1.Fill(dt1);
        GridView1.DataSource = dt1;
    ...
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["connection"]);
        SqlCommand CMD = new SqlCommand("select * from Customer_Entry where Custom_Add='" + TextBox15.Text + "' and Com_Id='" + company_id + "' ORDER BY Custom_Code asc", con);
        DataTable dt1 = new DataTable();
        SqlDataAdapter da1 = new SqlDataAdapter(CMD);
        da1.Fill(dt1);
        GridView1.DataSource = dt1;
    ...
    public static List<string> Searchaddress(string prefixText, int count)
    {
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.CommandText = "select distinct Custom_Add from Customer_Entry where Com_Id=@Com_Id and " +
                "Custom_Add like @Custom_Add + '%'";
                cmd.Parameters.AddWithValue("@Custom_Add", prefixText);
                cmd.Parameters.AddWithValue("@Com_Id", company_id);
                cmd.Connection = conn;
                conn.Open();
                List<string> customers = new List<string>();
                using (SqlDataReader sdr = cmd.ExecuteReader())
                {
                    while (sdr.Read())
                    {
                        customers.Add(sdr["Custom_Add"].ToString());
                    }
                }
                conn.Close();
                return customers;
